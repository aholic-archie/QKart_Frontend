    ...
    const performCheckout = async (token, items, addresses) => {
        try {
            if (validateRequest(items, addresses)) {
                ...
                localStorage.setItem("balance", balance - total)
                ...
                <Grid item xs={12} md={9}>
                    <Box className="shipping-container" minHeight="100vh">
                        ...
                        <Box>
                            ...
                            </Typography> : addresses.all.map(({ _id, address }) =>
                                <div key={_id}
                                    className={"address-item " + (addresses.selected === _id ? "selected" : "not-selected")}
                                >
                                    <Typography variant="body1" gutterBottom onClick={() => {
                                        setAddresses({
                                            ...addresses,
                                            selected: _id
                                        })
                                    }}>{address}</Typography>
                                    <Button onClick={() => { deleteAddress(token, _id) }}><><DeleteIcon />DELETE</></Button>
                                </div>)}
                        </Box>
